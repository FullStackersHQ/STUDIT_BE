...
public class StudyRecruitService {
    ...
    public Long createRecruit(StudyRecruitRequest.Create request, Long leaderId) {
        ...
        StudyRecruit studyRecruit = StudyRecruit.builder()
                ...
                .recruitEndAt(request.getRecruitEndAt()).build();
    ...
    public Page<StudyRecruitResponse.Summary> getAllRecruits(Pageable pageable) {
        Page<StudyRecruit> recruits = studyRecruitRepository
.findByStatusOrderByRecruitStartAtDesc(RecruitStatus.ACTIVE, pageable);
...
return recruits.map(studyRecruit -> StudyRecruitResponse.Summary.builder()
                ...
                .currentMembers(studyRegisterRepository
.countByStudyRecruitAndStatus(studyRecruit, RegisterStatus.REGISTER))
    ...
    public Page<StudyRecruitResponse.Summary> getSearchRecruits(
        ...
        Page<StudyRecruit> searchRecruits = studyRecruitRepository.findByFilters
                (title, tagsValue, category, minDepositValue, maxDepositValue,
                        minGoalTimeValue, maxGoalTimeValue, pageable);
...
.countByStudyRecruitAndStatus(studyRecruit, RegisterStatus.REGISTER))
                .maxMembers(studyRecruit.getMaxMembers())
                .status(studyRecruit.getStatus().name()).build());}
...
.countByStudyRecruitAndStatus(studyRecruit, RegisterStatus.REGISTER))
    ...
    // 스터디 모집글 수정
    public Long updateRecruit() {return null;}
    // 스터디 모집글 삭제
    public Long deleteRecruit() {return null;}
    // 스터디 가입
    public Long studyRegister() {return null;}
    // 스터디 가입 철회
    public Long withdrawRegister() {return null;}}